...
public class URLBuilder {
	...
	public URLBuilder() {
		this((URL) null, Charsets.UTF8_CS);
	}
	...
	public void addPara(KeyValuePair<String, String> pair) {
		this.addPara(pair.key, pair.value);
	}

	public synchronized void addPara(String key, String value) {
		if (key == null) {
			BmtLogger.instance().log(LogLevel.Warning, "url para key can not be null");
			return;
		}
		key = key.trim();
	...
	public String getParaString() {
		...
		for (KeyValuePair<String, String> pair : pairs) {
			...
			try {
				sb.append(URLEncoder.encode(pair.key.trim(), csName));
			} catch (Exception e) {
				BmtLogger.instance().log(LogLevel.Error, "unsupport charset %s", csName);
				sb.append(pair.key.trim());
			}
			sb.append('=');
			if (null != pair.value) {
				try {
					String enc = URLEncoder.encode(pair.value.trim(), csName);
					sb.append(enc);
				} catch (Exception e) {
					BmtLogger.instance().log(LogLevel.Error, "unsupport charset %s", csName);
