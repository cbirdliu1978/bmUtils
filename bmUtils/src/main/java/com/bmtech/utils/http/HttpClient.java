...
public class HttpClient implements Runnable {
	...
	/**
	 * set datas to post, if not null setted, will use post to connect HTTP
	 * SERVER
	 * 
	 * @param bs
	 */
	...
	public void run() {
		try {
			...
			BmtLogger.instance().log(e, "HttpClient reading fail for url %s",
					url);
		}
	}
		...
		try {
			while (true) {
				...
				if (stat >= 300 && stat <= 307 && stat != 306 && stat != 304) {
					...
					Iterator<Entry<String, List<String>>> itr = crl
							.getHeadInfo().entrySet().iterator();
					while (itr.hasNext()) {
					...
					if (lst.size() == 0) {
						throw new IOException(
								"got no trace url when got httpcode:" + stat
										+ " for url " + crl.getURL());
					}
					String u = lst.get(0);
