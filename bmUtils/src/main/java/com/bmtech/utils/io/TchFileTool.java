...
/**
 * this class define a model to serialize data: using a file to keep a special token, only read/write this token
 * 
 * @author beiming
 * 
 */
public class TchFileTool {
	...
	public String read(String id) {
		...
		if (file.exists()) {
			try {
				...
				BmtLogger.instance().log(e, "can not read a touch action, id = %s", id);
				return null;
			}
		} else {
			return null;
	 ...
	 * 
	 * @param id
	 * @return the number readed, if nothing read or format is not a number, return 0
	 */
	public int readInt(String id, int dft) {
		...
		if (str == null) {
			...
			try {
				...
				BmtLogger.instance().log(e, "can not parse a touched number, id = %s, token = %s", id, str);
				return dft;
			}
		}
	}
	...
	public boolean write(String id, String token) {
		...
		try {
			...
			LineWriter lw = new LineWriter(file, false, Charset.forName("utf-8"));
			lw.writeLine(token);
			lw.close();
			return true;
		} catch (Exception e) {
			BmtLogger.instance().log(e, "can not make a touch action, id = %s, token = %s", id, token);
			return false;
		}
	}
	...
	public static String get(String path, String id, String dft) {
		TchFileTool tft = new TchFileTool(path);
		String ret = tft.read(id);
		if (ret == null)
			ret = dft;
		return ret;
	}
